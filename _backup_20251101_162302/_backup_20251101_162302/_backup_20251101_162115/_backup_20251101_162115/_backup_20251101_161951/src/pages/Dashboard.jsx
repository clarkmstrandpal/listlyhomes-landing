  ...
  return (
    <motion.div variants={{ hidden: { opacity: 0, y: 8 }, show: { opacity: 1, y: 0 } }}
      ...
      <div className="text-sm text-gray-600 mt-1">
        {lead.price ? `$${Number(lead.price).toLocaleString()}` : "—"} · {lead.beds ?? "?"} bd · {lead.baths ?? "?"} ba
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        {!lead.claimed && lead.status !== "claimed" ? (
          ...
          <span className="px-2 py-1 rounded bg-green-100 text-green-800 text-sm">Claimed ✔</span>
        )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setFilters((f) => ({ ...f, status: "" }))}
            className={`rounded-full px-3 py-1.5 text-sm border ${filters.status === "" ? "bg-slate-900 text-white border-slate-900" : "bg-white hover:bg-slate-50"}`}>
          ...
          <button onClick={() => setFilters((f) => ({ ...f, status: "claimed" }))}
            className={`rounded-full px-3 py-1.5 text-sm border ${filters.status === "claimed" ? "bg-emerald-600 text-white border-emerald-600" : "bg-white hover:bg-slate-50"}`}>
